        ...
        <div class="flex flex-col w-[60rem] lg:w-[20rem] pt-5 px-4 ms-3">
            <div class="flex flex-row py-3">
                <a href="{{auth()->user()->username}}" class="w-12 h-12 rounded-full border-gray-300">
                    <img src="{{auth()->user()->image == 'https://ui-avatars.com/api/?name='.$post->user->name ? auth()->user()->image : asset('storage/' .auth()->user()->image)}}" alt="" class="h-12 w-12 border-2 border-sky-600 rounded-full">
                </a>
                ...
                @foreach ($suggested_users as $suggested_user)
                    <div class="flex flex-row items-center mt-4">
                        <a href="/{{$suggested_user->username}}" class="me-3">
                            <img src="{{$suggested_user->image == 'https://ui-avatars.com/api/?name='.urlencode($suggested_user->name) ? $suggested_user->image : asset('storage/' .$suggested_user->image)}}" alt="" class="h-9 w-9 border-2 border-red-600 rounded-full">
                        </a>
                        <div class="flex flex-col ms-2">
